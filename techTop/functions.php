...
add_action('wp_ajax_mini_remove_from_wishlist', 'mini_remove_from_wishlist');
function mini_remove_from_wishlist() {
	if ($_POST['sku']) {
		$data = User::remove_from_wishlist($_POST['sku']);
		if ($data->Status && $data->Status == 'OK') {
			$wishlist_data = User::get_wishlist('ASC');
        	$_SESSION['wishlist'] = $wishlist_data;
			$total = count($wishlist_data->OutTab);
			echo json_encode([
				'total' => $total
			]);
		} else {
...
function add_to_wishlist() {
    if ($_POST['sku']) {
	    $data = User::add_to_wishlist($_POST['sku']);
	    if ($data->Status && $data->Status == 'OK') {
			$wishlist_data = User::get_wishlist('ASC');
        	$_SESSION['wishlist'] = $wishlist_data;
		    echo $data->Status;
...
function mini_wishlist_fragments_refreshed() {
	$fragments['popup']  = get_template_part( 'template-parts/mini-wishlist' );
	wp_die();
}

add_action('wp_ajax_mini_wishlist_fragments_refreshed', 'mini_wishlist_fragments_refreshed');

function mini_wishlist_count_refreshed() {
	$wishlist_data = $_SESSION['wishlist'];
	$response = "";
	if (count($wishlist_data->OutTab) > 0) {
		$response = "<span>" . count($wishlist_data->OutTab). '</span>';
	}
	echo $response;
	wp_die();
}

add_action('wp_ajax_mini_wishlist_count_refreshed', 'mini_wishlist_count_refreshed');
